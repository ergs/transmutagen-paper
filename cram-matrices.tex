...
This is particularly and issue for eigenvalues with nonnegative real parts,
which, when small, result in small error but when large can produce enormous
error. See Figure~\ref{fig:error-plot}. It can also be an issue for imaginary
eigenvalues. In short, the CRAM approximation by construction only has minimal
error on the negative real axis. The error stays small near the negative real
axis but increases away from it.
