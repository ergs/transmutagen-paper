...
% TODO: Should c be
% tn = cos(2*(n - 1)/n*pi/2)
% c = nsolve((1 + tn)/(1 - tn)*((1 + tn)/(1 - tn)).subs(n, n - 1) - 1, 0.6)
% See Page 393 of Carpenter, Ruttan, and Varga paper
...
$[0, \infty)$, for degree 16.

\begin{figure}[!ht]
\centering
\resizebox{\textwidth}{!}{\input{cram-plot.pgf}}
\caption{Left: plot of $\hat{r}_{16, 16}\left(c\frac{t+1}{t-1}\right) -
  e^{-c\frac{t+1}{t-1}}$; right: plot of $\hat{r}_{16, 16}(t) -
e^{-t}$ ($c=0.6\cdot16=9.6$).}
\label{fig:cram-plot}
\end{figure}
