...
$(-1, 1)$.\footnote{$T_n(x)$ is the $n$th Chebyshev polynomial of the first
  ...
  roots of $T_n(x)$ are
  $\cos{\left (\frac{2k - 1}{n}\frac{\pi}{2} \right )},\,k=1\ldots n$.} We
found that convergence can take more than twice as many steps when random
initial points are used instead of the Chebyshev nodes. \todo{verify this.}
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \RETURN $r|_{t=\frac{t - c}{t + c}}$ \COMMENT{translate $[-1, 1)$ back to $[0, \infty)$ and normalize $q_0=1$}
  \end{algorithmic}
\end{algorithm}
